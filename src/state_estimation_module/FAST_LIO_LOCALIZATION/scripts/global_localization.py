...
def rotate_pose_msg(pose_msg):
    ...
    # 顺时针旋转45度的旋转矩阵
    rotation_matrix_45 = tf.transformations.rotation_matrix(np.radians(45), [0, 0, 1])
    ...
    # 在旋转矩阵上应用顺时针旋转45度的变换
    rotation_matrix_rotated = np.dot(rotation_matrix, rotation_matrix_45)

    # 将旋转矩阵转换回四元数
    quaternion_rotated = rotation_matrix_to_quaternion(rotation_matrix_rotated)

    # 更新姿态信息
    pose_msg.pose.orientation = quaternion_rotated
...
def global_localization(pose_estimation):
    ...
    transformation, _ = registration_at_scale(scan_tobe_mapped, global_map_in_FOV, 
                                             initial=pose_estimation, scale=8)
    rospy.loginfo('first_fitness:{}'.format(_)) 

    
    # 精配准

    transformation, fitness = registration_at_scale(scan_tobe_mapped, global_map_in_FOV, 
                                                    initial=transformation, scale=2)

 
    rospy.loginfo("the fitness:{}".format(fitness) )
...
def initialize_global_map(pc_msg):
    ...
    rospy.loginfo('Global pcd map received.')
...
if __name__ == '__main__':
    ...
    while not initialized:
        ...
        #if cur_scan:
        #    initialized = global_localization(pose_msg)
