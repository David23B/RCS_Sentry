...
void terrainCloudHandler(const sensor_msgs::PointCloud2ConstPtr& terrainCloud2)
{
  if (useTerrainAnalysis) {
    ...
    }//点云裁剪，只保留车辆附近 dis<adjacentRange 的点云

    terrainCloudDwz->clear();
    terrainDwzFilter.setInputCloud(terrainCloudCrop);
    terrainDwzFilter.filter(*terrainCloudDwz);

    newTerrainCloud = true;//表示之后处理仅使用地面分割后的点云数据
  }
}
...
void dynamiccallback(local_planner::LocalPlannerConfig &config, uint32_t level){
...
int main(int argc, char** argv)
{
  ...
  while (status) {
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      // 遍历所有采样路径集合，选取出一条可通行且代价最小的路径
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        // 初始化
        for (int i = 0; i < 36 * pathNum; i++) {
          clearPathList[i] = 0;    // 清除掉的路径
          pathPenaltyList[i] = 0;  // 路径的惩罚值
        }
        for (int i = 0; i < 36 * groupNum; i++) {
          clearPathPerGroupScore[i] = 0;  // 每组路径的得分
        }
        ...
        for (int i = 0; i < plannerCloudCropSize; i++) {
          ...
          float h = plannerCloudCrop->points[i].intensity;//点云与分割地面的相对高度
